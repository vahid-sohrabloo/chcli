...
func TestSmokeDetails(t *testing.T) {
	...
	must := func(sql string) {
		if err := c.Exec(ctx, sql); err != nil { t.Fatalf("%s: %v", sql, err) }
	}
	...
	if err != nil { t.Fatalf("partitions: %v", err) }
	if len(ps) == 0 { t.Fatal("expected at least one partition") }
	pd, err := FetchPartitionDetail(ctx, c, "detail_smoke", "t", ps[0].Name)
	if err != nil { t.Fatalf("partition detail: %v", err) }
	...
	if err != nil { t.Fatalf("parts: %v", err) }
	if len(prs) == 0 { t.Fatal("expected at least one part") }
	part, err := FetchPartDetail(ctx, c, "detail_smoke", "t", prs[0].Name)
	if err != nil { t.Fatalf("part detail: %v", err) }
